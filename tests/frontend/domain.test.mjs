...
test('external URL validation rejects unsafe schemes, credentials, and private hosts',()=>{assert.equal(safeExternalUrl('javascript:alert(1)'),null);assert.equal(safeExternalUrl('https://user:pass@example.com'),null);assert.equal(safeExternalUrl('https://localhost/evidence'),null);assert.equal(safeExternalUrl('https://192.168.1.2/evidence'),null);assert.equal(safeExternalUrl('https://example.com/a'),'https://example.com/a')})
